...
function build_release() {
	...
	ARMS=(5 6 7)
	for v in ${ARMS[@]}; do
		env CGO_ENABLED=0 GOOS=linux GOARCH=arm GOARM=$v go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -tags "$TAGS" -o shadowsocks_linux_arm${v}  github.com/ccsexyz/shadowsocks-go/cmd/shadowsocks
	done
	env CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -tags "$TAGS" -o shadowsocks_linux_arm64  github.com/ccsexyz/shadowsocks-go/cmd/shadowsocks
	tar -zcf shadowsocks-linux-arm-$VERSION.tar.gz shadowsocks_linux_arm*
	$SHA256 shadowsocks-linux-arm-$VERSION.tar.gz

	#MIPS32LE
	env CGO_ENABLED=0 GOOS=linux GOARCH=mipsle go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -tags "$TAGS" -o shadowsocks_linux_mipsle github.com/ccsexyz/shadowsocks-go/cmd/shadowsocks
	env CGO_ENABLED=0 GOOS=linux GOARCH=mips go build -ldflags "$LDFLAGS" -gcflags "$GCFLAGS" -tags "$TAGS" -o shadowsocks_linux_mips github.com/ccsexyz/shadowsocks-go/cmd/shadowsocks
